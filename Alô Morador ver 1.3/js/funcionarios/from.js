...
function montaTr(funcionario){
	...
	funcionarioTr.appendChild(montaTd(funcionario.cf, "info-cf"));
...
function validaFuncionario(funcionario) {
    ...
    // Verificação para garantir que CF contenha apenas números
    if (!/^\d+$/.test(funcionario.cf)) erros.push("O CF deve conter apenas números.");
    ...
    // if (!validarRa(funcionario.cf)) erros.push("CF é inválido.");
